...
DECLARE
  ...
  v_rows INTEGER := 0;
BEGIN
  SELECT id INTO v_consensus_source_id FROM public.market_signal_source WHERE source_code = ''market_consensus'';
  SELECT id INTO v_internal_source_id FROM public.market_signal_source WHERE source_code = ''internal_realized_sale'';
  ...
  INSERT INTO public.market_signal (
    source_id, sku_id, mpn, condition_grade, channel, signal_type,
    observed_price, observed_price_min, observed_price_max, sample_size,
    vat_treatment, source_confidence, freshness_score, observed_at, metadata
  )
  SELECT
  ...
  INSERT INTO public.market_price_snapshot (
    source_id, sku_id, mpn, condition_grade, channel,
    price, price_low, price_high, currency, vat_treatment,
    sample_size, confidence_score, freshness_score, captured_at, metadata
  )
  WITH weighted_signals AS (
    SELECT
      ...
      GREATEST(0.05, LEAST(1.00,
        COALESCE(ms.freshness_score, 1.00 - (EXTRACT(EPOCH FROM (now() - ms.observed_at)) / 31536000.0))
      )) AS freshness_part
  ...
  grouped AS (
    SELECT
      sku_id, mpn, condition_grade, channel,
  ...
  all_channel AS (
    SELECT
      sku_id, mpn, condition_grade, ''all'' AS channel,
      ROUND((SUM(observed_price * signal_weight) / NULLIF(SUM(signal_weight), 0))::numeric, 2) AS price,
      ROUND(MIN(observed_price_min)::numeric, 2) AS price_low,
      ROUND(MAX(observed_price_max)::numeric, 2) AS price_high,
      SUM(sample_size)::integer AS sample_size,
  ...
  SELECT
    v_consensus_source_id, c.sku_id, c.mpn, c.condition_grade, c.channel,
    c.price, c.price_low, c.price_high, ''GBP'',
    CASE
      WHEN c.vat_treatments ? ''inclusive'' THEN ''inclusive''
      WHEN c.vat_treatments ? ''exclusive'' THEN ''exclusive''
      ELSE ''unknown''
    END,
    c.sample_size, c.confidence_score, c.freshness_score, now(),
    jsonb_build_object(
