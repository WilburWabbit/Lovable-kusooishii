...
-- The wrapped quote breakdown stores risk_reserve_rate as a percent value;
-- 0.5 means 0.5%, so pricing math must use 0.005, not 0.5.
-- Replaces commerce_quote_price and create_price_decision_snapshot with the
-- same VAT-aware implementations, correcting risk-rate normalization.
-- Listing prices are customer-facing gross prices. Floor/target economics must
-- show output VAT payable, recoverable input VAT on fees, and the net position.
