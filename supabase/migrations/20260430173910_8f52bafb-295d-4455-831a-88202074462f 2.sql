...
BEGIN
  SELECT * INTO v_line FROM public.sales_order_line WHERE id = p_sales_order_line_id FOR UPDATE;
  IF NOT FOUND THEN
  ...
  IF v_stock_unit_id IS NOT NULL AND v_reversal_amount <> 0 THEN
    INSERT INTO public.stock_cost_event (
      stock_unit_id, sales_order_id, sales_order_line_id, event_type,
      amount, currency, costing_method, source, idempotency_key, metadata, event_at
    )
    VALUES (
      v_stock_unit_id, v_line.sales_order_id, v_line.id, ''manual_correction'',
      -ABS(v_reversal_amount), ''GBP'',
...
BEGIN
  WITH target_skus AS (
    SELECT id FROM public.sku WHERE p_sku_id IS NULL OR id = p_sku_id
  ),
  stats AS (
    SELECT
      su.sku_id,
