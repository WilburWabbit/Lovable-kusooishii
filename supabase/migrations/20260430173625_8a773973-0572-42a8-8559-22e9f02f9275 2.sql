...
BEGIN
  INSERT INTO public.reconciliation_case (
    case_type, severity, sales_order_id, sales_order_line_id,
    related_entity_type, related_entity_id, suspected_root_cause,
    recommended_action, evidence
  )
  SELECT
    ''unallocated_order_line'', ''high'', sol.sales_order_id, sol.id,
    ...
    jsonb_build_object(''sku_id'', sol.sku_id, ''quantity'', sol.quantity, ''economics_status'', sol.economics_status)
  ...
  SELECT
    ''missing_cogs'', ''high'', sol.sales_order_id, sol.id,
    ''sales_order_line'', sol.id,
    ''Sale line has no finalized COGS.'',
    ''Allocate stock or approve a manual costing exception before final accounting close.'',
    jsonb_build_object(''sku_id'', sol.sku_id, ''stock_unit_id'', sol.stock_unit_id, ''costing_method'', sol.costing_method, ''economics_status'', sol.economics_status)
  FROM public.sales_order_line sol
  WHERE (p_sales_order_id IS NULL OR sol.sales_order_id = p_sales_order_id)
    AND sol.stock_unit_id IS NOT NULL
    AND (sol.cogs_amount IS NULL OR sol.cogs_amount = 0 OR sol.economics_status IN (''needs_allocation'', ''incomplete''))
    AND NOT EXISTS (
      SELECT 1 FROM public.reconciliation_case rc
  ...
  INSERT INTO public.reconciliation_case (
    ...
    suspected_root_cause, recommended_action, amount_expected, evidence
  )
  SELECT
    ''unmatched_payout_fee'', ''medium'', ''payout_fee'', pf.id,
    ...
    pf.amount,
    jsonb_build_object(''payout_id'', pf.payout_id, ''external_order_id'', pf.external_order_id, ''channel'', pf.channel, ''fee_category'', pf.fee_category)
  ...
  INSERT INTO public.reconciliation_case (
    case_type, severity, sales_order_id, suspected_root_cause,
    recommended_action, amount_expected, amount_actual, variance_amount, evidence
  )
  WITH expected AS (
  ...
  INSERT INTO public.reconciliation_case (
    case_type, severity, sales_order_id, suspected_root_cause,
    recommended_action, amount_expected, amount_actual, variance_amount, due_at, evidence
  )
  WITH expected AS (
  ...
  SELECT
    ''missing_payout'', ''medium'', e.sales_order_id,
    ...
    e.expected_amount, 0, e.expected_amount,
    so.created_at + interval ''7 days'',
    jsonb_build_object(''origin_channel'', so.origin_channel, ''order_number'', so.order_number, ''order_created_at'', so.created_at)
  FROM expected e
  JOIN public.sales_order so ON so.id = e.sales_order_id
  WHERE NOT EXISTS (
      SELECT 1 FROM public.actual_settlement_line asl WHERE asl.sales_order_id = e.sales_order_id
    )
    AND NOT EXISTS (
      SELECT 1 FROM public.reconciliation_case rc
  ...
  INSERT INTO public.reconciliation_case (
    case_type, severity, sales_order_id, related_entity_type, related_entity_id,
    suspected_root_cause, recommended_action, amount_expected, evidence
  )
  SELECT
    ''unpaid_program_accrual'', ''medium'', spa.sales_order_id,
    ''sales_program_accrual'', spa.id,
    ...
    jsonb_build_object(''sales_program_id'', spa.sales_program_id, ''status'', spa.status, ''currency'', spa.currency)
  ...
  INSERT INTO public.reconciliation_case (
    case_type, severity, sales_order_id, related_entity_type, related_entity_id,
    suspected_root_cause, recommended_action, evidence
  )
  SELECT
    ''qbo_posting_gap'', ''high'', pi.entity_id, ''posting_intent'', pi.id,
    ''QBO posting intent failed.'',
    ''Review the posting error, fix the source data or QBO mapping, then retry the outbox processor.'',
    jsonb_build_object(''action'', pi.action, ''retry_count'', pi.retry_count, ''last_error'', pi.last_error, ''idempotency_key'', pi.idempotency_key)
  ...
  SELECT
    ''qbo_posting_gap'', ''medium'', so.id, ''sales_order'', so.id,
    ...
    jsonb_build_object(''order_number'', so.order_number, ''origin_channel'', so.origin_channel, ''accounting_event_count'', COUNT(ae.id))
  ...
  WHERE (p_sales_order_id IS NULL OR so.id = p_sales_order_id)
    ...
    AND NOT EXISTS (
      SELECT 1 FROM public.qbo_posting_reference qpr
  ...
  INSERT INTO public.reconciliation_case (
    case_type, severity, related_entity_type, suspected_root_cause,
    recommended_action, amount_actual, evidence
  )
  WITH duplicates AS (
    SELECT
      source_system, external_reference, category,
  ...
  SELECT
    ''duplicate_candidate'', ''medium'', ''actual_settlement_line'',
    ...
    d.total_amount,
    jsonb_build_object(''source_system'', d.source_system, ''external_reference'', d.external_reference, ''category'', d.category, ''duplicate_count'', d.duplicate_count, ''first_seen'', d.first_seen)
  FROM duplicates d
  WHERE NOT EXISTS (
    SELECT 1 FROM public.reconciliation_case rc
