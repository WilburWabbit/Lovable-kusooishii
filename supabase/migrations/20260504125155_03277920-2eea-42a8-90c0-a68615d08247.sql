-- Add manual channel availability holds and make quantity sync commands honor them.
...
BEGIN
  ...
  IF p_command_type = ''sync_quantity'' THEN
    ...
    v_quantity_marker := COALESCE(
      v_listing.availability_override_at,
      v_listing.synced_at,
      v_listing.updated_at,
      now()
    )::text;
