-- Mark historic orders made before 2026-04-28 as paid out.
--
-- "Paid out" in the app is represented by:
--   1. Order lifecycle closed: sales_order.v2_status = 'complete'
--   2. Sold/shipped/delivered units moved to stock_unit.v2_status = 'payout_received'
--   3. Rolling settlement monitor has actual settlement evidence matching the
--      expected settlement total, so old orders no longer appear as withheld.
...
SET v2_status = 'payout_received',
    completed_at = COALESCE(su.completed_at, now())
FROM historic_orders ho
WHERE su.order_id = ho.id
