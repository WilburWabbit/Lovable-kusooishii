...
BEGIN
  ...
  INSERT INTO public.price_override (
    price_decision_snapshot_id, sku_id, channel_listing_id, channel,
    override_type, old_price, new_price, reason_code, reason_note,
    approved_by, performed_by
  )
  VALUES (
    v_snapshot.id, v_snapshot.sku_id, v_snapshot.channel_listing_id, v_snapshot.channel,
    CASE
      WHEN COALESCE(v_snapshot.candidate_price, 0) < COALESCE(v_snapshot.floor_price, 0) THEN ''below_floor''
      ELSE ''margin_exception''
    END,
    v_snapshot.current_price, v_snapshot.candidate_price,
    v_reason_code, NULLIF(trim(COALESCE(p_reason_note, '''')), ''''),
    v_actor, auth.uid()
  )
