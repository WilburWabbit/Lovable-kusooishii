...
BEGIN
  ...
  -- Lock the requested stock unit by UID
  SELECT su.*
    INTO v_unit
  ...
  -- If the unit's SKU differs from the line's SKU, re-point the line
  IF v_unit.sku_id IS DISTINCT FROM v_line.sku_id THEN
    ...
    -- Safety: require at least the same MPN family (same product, different grade is OK)
  ...
  -- Mark the unit sold and bind it to the order
  ...
  -- Bind the unit to the line and copy its landed cost into COGS
  UPDATE public.sales_order_line
  SET stock_unit_id = v_unit.id,
      cogs = v_unit.landed_cost
  WHERE id = p_line_item_id;

  -- If all lines are now allocated, advance order status off needs_allocation
  SELECT count(*)
