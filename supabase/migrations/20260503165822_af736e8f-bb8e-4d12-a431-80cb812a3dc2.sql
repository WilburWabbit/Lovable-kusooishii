...
BEGIN
  ...
  IF FOUND THEN
    v_market_consensus := v_market_snapshot.consensus_price;
    v_market_confidence := COALESCE(v_market_snapshot.confidence_score, 0);
    v_market_channel := v_market_snapshot.channel;
  ELSE
    v_market_consensus := v_legacy_market_price;
    v_market_confidence := CASE WHEN v_legacy_market_price IS NOT NULL THEN 0.5 ELSE 0 END;
  END IF;

  IF v_market_consensus IS NOT NULL AND v_market_consensus > 0 THEN
    v_pre_undercut_market_price := v_market_consensus * COALESCE(v_condition_multiplier, 1);
    v_minimum_undercut := GREATEST(v_pre_undercut_market_price * v_min_undercut_pct, v_min_undercut_amount);
