...
BEGIN
  ...
  ORDER BY CASE
    ...
    WHEN name = 'SUBLEDGER_CRON_SECRET' THEN 3
    WHEN name = 'internal_cron_secret' THEN 4
    ELSE 5
  END
  LIMIT 1;

  IF NULLIF(v_internal_secret, '') IS NULL THEN
    RAISE EXCEPTION 'Missing internal secret for scheduled subledger job. Store it in vault as subledger_scheduled_jobs_secret or internal_cron_secret.';
  END IF;
