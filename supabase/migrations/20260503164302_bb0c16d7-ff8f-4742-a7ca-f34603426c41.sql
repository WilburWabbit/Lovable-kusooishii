...
BEGIN
  SELECT sk.sku_code, sk.price, sk.sale_price, sk.market_price, sk.condition_grade::int, sk.mpn,
         COALESCE(pr.weight_kg, 0), pr.length_cm, pr.width_cm, pr.height_cm
  INTO v_sku_code, v_sku_price, v_sku_sale_price, v_legacy_market_price, v_condition_grade, v_mpn,
  ...
  SELECT id INTO v_channel_policy_id FROM public.channel_price_policy WHERE price_policy_id = v_policy_id AND channel = v_channel AND active = true LIMIT 1;

  SELECT COALESCE((SELECT value FROM public.selling_cost_defaults WHERE key = ''minimum_profit_amount''), 1) INTO v_min_profit;
  SELECT COALESCE((SELECT value FROM public.selling_cost_defaults WHERE key = ''minimum_margin_rate''), 0.15) INTO v_min_margin;
  ...
  SELECT COALESCE(market_undercut_min_pct, 0), COALESCE(market_undercut_min_amount, 0), market_undercut_max_pct, market_undercut_max_amount
  ...
  SELECT COALESCE((
    SELECT srt.cost FROM public.shipping_rate_table srt
    WHERE srt.channel = ''default'' AND srt.tier = v_active_tier AND srt.destination = ''domestic'' AND srt.active = true
      AND srt.max_weight_kg >= COALESCE(v_weight_kg, 0)
      AND (NOT v_has_dimensions OR ((srt.max_length_cm IS NULL OR srt.max_length_cm >= v_length_cm) AND (srt.max_width_cm IS NULL OR srt.max_width_cm >= v_width_cm) AND (srt.max_depth_cm IS NULL OR srt.max_depth_cm >= v_height_cm)))
    ORDER BY srt.cost ASC LIMIT 1
  ), 0) INTO v_default_shipping_cost;
  ...
  FOR v_fee IN SELECT * FROM public.channel_fee_schedule cfs WHERE cfs.channel = v_channel AND cfs.active = true LOOP
  ...
  FOR i IN 1..5 LOOP
    ...
    FOR v_fee IN SELECT * FROM public.channel_fee_schedule cfs WHERE cfs.channel = v_channel AND cfs.active = true LOOP
      v_fee_base := v_floor;
      IF v_fee.applies_to = ''sale_plus_shipping'' THEN v_fee_base := v_floor + v_shipping_cost;
      ELSIF v_fee.applies_to = ''sale_price_inc_vat'' THEN v_fee_base := v_floor * 1.2; END IF;
    ...
    IF v_needed_price <= v_floor + 0.01 THEN EXIT; END IF;
  ...
  SELECT row_data.* INTO v_market_snapshot FROM (
    ...
    ORDER BY CASE WHEN mps.channel = v_channel THEN 1 WHEN mps.channel = ''all'' THEN 2 ELSE 3 END, mps.captured_at DESC
    LIMIT 1
  ) row_data;
  ...
  ELSIF v_mpn IS NOT NULL THEN
    SELECT bec.current_value INTO v_market_consensus FROM public.brickeconomy_collection bec
    WHERE bec.item_number IN (v_mpn, regexp_replace(v_mpn, ''-\d+$'', '''')) AND bec.current_value IS NOT NULL
  ...
  IF v_market_consensus IS NOT NULL THEN
    ...
    IF v_max_undercut_pct IS NOT NULL OR v_max_undercut_amount IS NOT NULL THEN
      v_maximum_undercut := GREATEST(COALESCE(v_pre_undercut_market_price * v_max_undercut_pct, 0), COALESCE(v_max_undercut_amount, 0));
      IF v_maximum_undercut <= 0 THEN v_maximum_undercut := NULL; END IF;
    END IF;
    v_applied_market_undercut := CASE WHEN v_maximum_undercut IS NULL THEN v_minimum_undercut ELSE LEAST(v_minimum_undercut, v_maximum_undercut) END;
    v_target := floor(v_pre_undercut_market_price - v_applied_market_undercut) + 0.99;
    IF v_target > (v_pre_undercut_market_price - v_applied_market_undercut) THEN v_target := v_target - 1; END IF;
    IF v_target < v_floor THEN v_target := v_floor; v_target_floor_clamped := true; END IF;
  ELSE
    v_target := v_floor;
    v_warnings := v_warnings || jsonb_build_array(''missing_market_consensus'');
  END IF;

  IF p_candidate_price IS NOT NULL AND p_candidate_price > 0 THEN v_gross_price := ROUND(p_candidate_price, 2);
  ...
  FOR v_fee IN SELECT * FROM public.channel_fee_schedule cfs WHERE cfs.channel = v_channel AND cfs.active = true LOOP
    ...
    IF v_fee.applies_to = ''sale_plus_shipping'' THEN v_fee_base := v_gross_price + v_shipping_cost;
    ELSIF v_fee.applies_to = ''sale_price_inc_vat'' THEN v_fee_base := v_gross_price * 1.2; END IF;
    v_fee_amount := (v_fee_base * (COALESCE(v_fee.rate_percent, 0) / 100)) + COALESCE(v_fee.fixed_amount, 0);
    IF v_fee.min_amount IS NOT NULL AND v_fee_amount < v_fee.min_amount THEN v_fee_amount := v_fee.min_amount; END IF;
    IF v_fee.max_amount IS NOT NULL AND v_fee_amount > v_fee.max_amount THEN v_fee_amount := v_fee.max_amount; END IF;
    v_total_channel_fees := v_total_channel_fees + v_fee_amount;
  ...
  RETURN jsonb_build_object(
    ...
    ''fee_components'', jsonb_build_object(''estimated_fees'', v_total_channel_fees, ''fee_rate'', v_effective_fee_rate, ''fixed_fee_amount'', v_fixed_fee_costs),
    ...
    ''delivery_cost'', ROUND(v_shipping_cost, 2), ''cost_base'', ROUND(v_cost_base, 2),
    ...
    ''blocking_reasons'', v_blocking, ''warning_reasons'', v_warnings,
    ...
    ''breakdown'', jsonb_build_object(
      ...
      ''total_fee_rate'', ROUND(v_effective_fee_rate * 100, 2), ''fixed_fee_costs'', ROUND(v_fixed_fee_costs, 2),
      ''estimated_fees_at_target'', v_total_channel_fees, ''estimated_net_at_target'', v_estimated_net,
      ''risk_reserve_rate'', ROUND(v_risk_rate * 100, 2), ''min_profit'', v_min_profit,
      ''min_margin'', ROUND(v_min_margin * 100, 2), ''market_confidence'', ROUND(v_market_confidence, 2),
...
BEGIN
  ...
  SELECT id INTO v_policy_id FROM public.price_policy WHERE policy_code = ''default'';
  SELECT id INTO v_channel_policy_id FROM public.channel_price_policy WHERE price_policy_id = v_policy_id AND channel = v_channel AND active = true LIMIT 1;

  IF p_sales_program_code IS NOT NULL THEN
    SELECT id INTO v_program_id FROM public.sales_program WHERE program_code = p_sales_program_code;
  END IF;

  INSERT INTO public.price_decision_snapshot (
    sku_id, channel_listing_id, price_policy_id, channel_price_policy_id, sales_program_id,
    channel, currency, candidate_price, current_price, floor_price, target_price, ceiling_price,
    market_consensus_price, carrying_value_basis, packaging_cost, delivery_cost, estimated_fees,
    estimated_program_discount, estimated_program_commission, expected_gross, expected_net_before_cogs,
    expected_margin_amount, expected_margin_rate, confidence_score, recommendation,
    blocking_reasons, override_required, inputs, calculation_version, created_by
  )
  VALUES (
    p_sku_id, p_channel_listing_id, v_policy_id, v_channel_policy_id, v_program_id,
    v_channel, ''GBP'',
    (v_quote->>''gross_price'')::numeric, (v_quote->>''gross_price'')::numeric,
    (v_quote->>''floor_price'')::numeric, (v_quote->>''target_price'')::numeric, (v_quote->>''ceiling_price'')::numeric,
    NULLIF(v_quote->>''market_consensus_price'', '''')::numeric,
    (v_quote->>''cogs_or_carrying_value'')::numeric, (v_quote->>''packaging_cost'')::numeric,
    (v_quote->>''delivery_cost'')::numeric, (v_quote->>''estimated_fees'')::numeric,
    (v_quote->>''discounts'')::numeric, (v_quote->>''program_commission'')::numeric,
    (v_quote->>''gross_price'')::numeric, (v_quote->>''estimated_net'')::numeric,
    (v_quote->>''expected_net_margin'')::numeric, NULLIF(v_quote->>''expected_net_margin_rate'', '''')::numeric,
    (v_quote->>''confidence_score'')::numeric,
    CASE WHEN (v_quote->>''override_required'')::boolean THEN ''review''
         WHEN jsonb_array_length(COALESCE(v_quote->''warning_reasons'', ''[]''::jsonb)) > 0 THEN ''review''
         ELSE ''publish'' END,
    v_quote->''blocking_reasons'', (v_quote->>''override_required'')::boolean,
    v_quote, ''commerce_quote_v2'', p_actor_id
...
DROP TRIGGER IF EXISTS coalesce_market_signal_source_metadata ON public.market_signal_source;
...
ALTER TABLE public.market_signal_source ALTER COLUMN metadata SET DEFAULT '{}'::jsonb;
UPDATE public.market_signal_source SET metadata = '{}'::jsonb WHERE metadata IS NULL;