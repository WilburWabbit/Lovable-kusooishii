...
BEGIN
  ...
  VALUES (
    ''qbo'', ''create_payout_deposit'', ''payout'', p_payout_id,
    ''qbo:create_payout_deposit:'' || p_payout_id::text, ''pending'',
    jsonb_build_object(
  ...
  SET payload = EXCLUDED.payload,
      status = CASE WHEN posting_intent.status IN (''failed'', ''cancelled'') THEN ''pending'' ELSE posting_intent.status END,
      next_attempt_at = CASE WHEN posting_intent.status IN (''failed'', ''cancelled'') THEN now() ELSE posting_intent.next_attempt_at END,
      updated_at = now()
...
GRANT EXECUTE ON FUNCTION public.queue_qbo_payout_posting_intent(UUID) TO authenticated, service_role;
