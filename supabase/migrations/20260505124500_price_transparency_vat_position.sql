...
BEGIN
  ...
  END;
  v_risk_rate := CASE
    WHEN v_raw_risk_rate IS NULL THEN 0
    WHEN v_raw_risk_rate > 1 THEN v_raw_risk_rate / 100
    ELSE v_raw_risk_rate
  END;
  ...
  RETURN v_quote
    || jsonb_build_object(
      ...
      ''calculation_basis'', ''pool_wac_vat_v1'',
...
BEGIN
  ...
  SET
    ...
    calculation_version = ''pool_wac_vat_transparency_v1''
  WHERE id = v_snapshot_id;
