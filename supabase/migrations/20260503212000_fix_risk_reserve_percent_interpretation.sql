-- Fix risk reserve interpretation: 0.5 means 0.5%, not 50%.
-- Replaces commerce_quote_price with percent-normalized risk reserve math.
...
BEGIN
  ...
  -- risk_reserve_rate is an operator-entered percent value (e.g., 0.5 = 0.5%).
  -- Convert basis points-style entry to a decimal multiplier for pricing math.
  v_risk_rate := GREATEST(COALESCE(v_risk_reserve_rate, 0), 0) / 100;
