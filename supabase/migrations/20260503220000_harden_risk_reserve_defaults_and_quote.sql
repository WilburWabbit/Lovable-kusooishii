-- Repair risk reserve semantics and harden defaults for pricing.

-- Historical data fix: values like 0.5 were entered as "0.5%".
-- Convert to canonical decimal representation (0.005).
...
-- Ensure target prices respect floor prices and use the least discount
-- from market undercut policies (percentage vs absolute amount).

-- Rebalance pricing so market target and cost-recovery floor are separate signals.
-- The engine must not invent unmarketable target prices merely to protect margin.
...
DECLARE
  ...
  v_market_target_below_floor BOOLEAN := false;
...
BEGIN
  ...
  INTO v_min_profit;
  SELECT COALESCE((SELECT value FROM public.selling_cost_defaults WHERE key = ''minimum_margin_rate''), 0.05)
  INTO v_min_margin;
  ...
  -- Backward-compatible normalization for risk reserve:
  -- - canonical decimal style: 0.005 = 0.5%
  -- - percent-style admin entry: 0.5 = 0.5% (auto-normalized)
  v_risk_rate := GREATEST(COALESCE(v_risk_reserve_rate, 0), 0);
  IF v_risk_rate >= 0.10 THEN
    v_risk_rate := v_risk_rate / 100;
    v_warnings := v_warnings || jsonb_build_array(''risk_reserve_percent_normalized'');
  END IF;
  ...
  -- Solve floor on the same gross-price basis operators see in the UI.
  -- Fees are applied once, risk reserve is applied once, and target is not
  -- clamped to this floor if market value cannot support it.
  FOR i IN 1..8 LOOP
  ...
  IF v_market_consensus IS NOT NULL THEN
    ...
    v_minimum_undercut := LEAST(v_market_anchor * COALESCE(v_min_undercut_pct, 0), COALESCE(v_min_undercut_amount, 0));
    IF v_max_undercut_pct IS NOT NULL OR v_max_undercut_amount IS NOT NULL THEN
      v_maximum_undercut := GREATEST(
        COALESCE(v_market_anchor * v_max_undercut_pct, 0),
    ...
    v_applied_market_undercut := CASE
      ...
      ELSE GREATEST(LEAST(v_minimum_undercut, v_maximum_undercut), 0)
    END;
  ...
  IF v_target < v_floor THEN
    v_market_target_below_floor := true;
  ...
  RETURN jsonb_build_object(
    ...
    ''cost_base'', ROUND(v_non_fee_costs, 2),
    ...
    ''breakdown'', jsonb_build_object(
      ...
      ''target_floor_clamped'', CASE WHEN v_market_target_below_floor THEN 1 ELSE 0 END,
      ''market_target_below_floor'', CASE WHEN v_market_target_below_floor THEN 1 ELSE 0 END
    )
  );
END;
';
