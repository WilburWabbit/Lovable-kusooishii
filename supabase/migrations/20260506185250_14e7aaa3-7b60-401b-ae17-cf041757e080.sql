-- Fix Issue Inbox false positives:
-- stock_unit.line_item_id is the purchase-line origin, not an active allocation marker.
-- Product routes in the admin app are MPN based, so issue links must not emit product UUID routes.
...
raw_issues AS (
  ...
  SELECT
    ...
    jsonb_build_object(
      ...
      'product', jsonb_build_object('product_id', p.id, 'mpn', p.mpn, 'name', p.name, 'sku_code', sk.sku_code),
    ...
    CASE
      WHEN p.mpn IS NOT NULL THEN '/admin/products/' || p.mpn
      WHEN sk.mpn IS NOT NULL THEN '/admin/products/' || sk.mpn
  ...
  SELECT
    ...
    jsonb_build_object(
      'outbound_command', jsonb_build_object('id', cmd.id, 'target_system', cmd.target_system, 'command_type', cmd.command_type, 'status', cmd.status, 'last_error', cmd.last_error),
