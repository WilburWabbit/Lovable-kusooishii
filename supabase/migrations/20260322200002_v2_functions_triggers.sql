-- ============================================================
-- Admin V2 — Migration 3: Functions, Triggers, Views
-- PO sequence, UID generation, cost apportionment,
-- grade reallocation, variant stats, FIFO, VAT.
-- ============================================================
...
-- ─── Function: auto-generate stock unit UID ───
...
DECLARE
  ...
  seq_num INTEGER;
BEGIN
  -- Only generate if batch_id is set and uid is not already provided
...
CREATE TRIGGER trg_v2_generate_uid
  ...
  FOR EACH ROW
  EXECUTE FUNCTION public.v2_generate_stock_unit_uid();

-- ─── Function: calculate apportioned costs for a batch ───
...
AS $$
DECLARE
  v_total_shared NUMERIC(12,2);
  v_total_unit_cost NUMERIC;
BEGIN
  -- Get total shared costs from the batch
  SELECT total_shared_costs INTO v_total_shared
  FROM public.purchase_batches
  WHERE id = p_batch_id;

  IF v_total_shared IS NULL OR v_total_shared = 0 THEN
    -- No shared costs — landed cost = unit cost
    UPDATE public.purchase_line_items
    SET apportioned_cost = 0,
        landed_cost_per_unit = unit_cost
    WHERE batch_id = p_batch_id;
  ELSE
    -- Sum of (unit_cost * quantity) across all line items
    SELECT COALESCE(SUM(unit_cost * quantity), 0) INTO v_total_unit_cost
    FROM public.purchase_line_items
    WHERE batch_id = p_batch_id;

    IF v_total_unit_cost > 0 THEN
      -- Apportion shared costs proportionally to unit cost
      UPDATE public.purchase_line_items
  ...
  -- Propagate landed cost to stock units in this batch
  UPDATE public.stock_unit su
  SET landed_cost = pli.landed_cost_per_unit
  FROM public.purchase_line_items pli
  WHERE su.line_item_id = pli.id
    AND su.batch_id = p_batch_id;
END;
$$;

-- ─── Function: reallocate costs by grade (relative sales value) ───
...
DECLARE
  ...
  v_total_expected_revenue NUMERIC;
  rec RECORD;
  -- Default grade ratios when market prices unavailable
  grade_ratio NUMERIC[] := ARRAY[1.0, 0.8, 0.6, 0.4];
BEGIN
  -- Total landed cost for the line item (quantity * landed_cost_per_unit)
  SELECT quantity * landed_cost_per_unit INTO v_total_landed
  FROM public.purchase_line_items
  WHERE id = p_line_item_id;

  IF v_total_landed IS NULL OR v_total_landed = 0 THEN
    RETURN;
  END IF;

  -- Calculate total expected revenue across all graded units
  -- Uses market_price from the sku table if available, else default ratios
  SELECT COALESCE(SUM(
    CASE
      WHEN sk.market_price IS NOT NULL AND sk.market_price > 0
        THEN sk.market_price
      ELSE grade_ratio[su.condition_grade::integer] * 100  -- baseline £100 * ratio
    END
  ), 0) INTO v_total_expected_revenue
  FROM public.stock_unit su
  LEFT JOIN public.sku sk ON sk.sku_code = (su.mpn || '.' || su.condition_grade::text)
  WHERE su.line_item_id = p_line_item_id
    AND su.condition_grade IS NOT NULL;

  IF v_total_expected_revenue = 0 THEN
    RETURN;
  END IF;

  -- Reallocate landed cost per unit based on grade's share of expected revenue
  FOR rec IN
    SELECT su.id AS unit_id,
           ...
           CASE
             WHEN sk.market_price IS NOT NULL AND sk.market_price > 0
               THEN sk.market_price
             ELSE grade_ratio[su.condition_grade::integer] * 100
  ...
  LOOP
    ...
    SET landed_cost = ROUND((rec.expected_price / v_total_expected_revenue) * v_total_landed, 2)
    WHERE id = rec.unit_id;
  END LOOP;
END;
$$;

-- ─── Function: recalculate variant stats (avg_cost, floor_price, cost_range) ───
...
DECLARE
  ...
  v_max NUMERIC(12,2);
  v_range TEXT;
  v_min_margin NUMERIC := 0.25;  -- 25% minimum margin
BEGIN
  -- Calculate stats from on-hand stock units
  SELECT
  ...
  -- Floor price = highest batch cost * (1 + minimum margin)
  IF v_floor IS NOT NULL THEN
    v_floor := ROUND(v_floor * (1 + v_min_margin), 2);
  END IF;
...
-- ─── Function: compute VAT from gross amount ───
...
AS $$
  SELECT
    ROUND(gross / 1.2, 2) AS net,
    gross - ROUND(gross / 1.2, 2) AS vat;
$$;

-- ─── Function: consume FIFO unit for a sale ───
...
BEGIN
  -- Select the oldest listed unit for this SKU
  SELECT su.* INTO v_unit
  FROM public.stock_unit su
  JOIN public.sku sk ON sk.id = su.sku_id
  WHERE sk.sku_code = p_sku_code
    AND su.v2_status = 'listed'
  ORDER BY su.created_at ASC
  LIMIT 1
  FOR UPDATE OF su;

  IF v_unit.id IS NULL THEN
    RAISE EXCEPTION 'No listed stock units available for SKU %', p_sku_code;
  END IF;
...
-- ─── Trigger: on grade change → recalculate variant stats + reallocate costs ───
...
BEGIN
  ...
  IF NEW.condition_grade IS NOT NULL AND NEW.mpn IS NOT NULL THEN
    ...
    -- If the grade just changed, also recalculate for the old SKU
...
CREATE TRIGGER trg_v2_on_grade_change
  ...
  FOR EACH ROW
  WHEN (OLD.condition_grade IS DISTINCT FROM NEW.condition_grade)
  EXECUTE FUNCTION public.v2_on_grade_change();

-- ─── View: variant stock summary ───
