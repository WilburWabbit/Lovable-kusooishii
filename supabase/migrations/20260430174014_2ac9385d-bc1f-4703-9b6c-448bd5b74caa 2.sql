...
BEGIN
  ...
  VALUES (
    ''qbo'', ''upsert_customer'', ''customer'', v_entity_id,
    ''qbo:upsert_customer:'' || v_idempotency_subject, ''pending'',
    COALESCE(p_payload, ''{}''::jsonb)
      || jsonb_build_object(''customer_id'', p_customer_id, ''queued_by'', v_actor, ''queued_at'', now())
  )
...
GRANT EXECUTE ON FUNCTION public.queue_qbo_customer_posting_intent(UUID, JSONB) TO authenticated, service_role;
