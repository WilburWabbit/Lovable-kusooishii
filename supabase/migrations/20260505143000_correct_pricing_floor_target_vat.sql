-- Correct pricing floor, target, and VAT treatment.
-- Floor is break-even only. Target carries commercial pricing policy and
-- minimum profit/margin safeguards. Listing prices remain gross prices.
-- All stock/packaging/delivery/channel fee costs are treated as gross paid
-- with reclaimable input VAT unless a future source explicitly stores net.
...
BEGIN
  ...
  SET
    ...
    calculation_version = ''pool_wac_vat_break_even_floor_v1''
  WHERE id = v_snapshot_id;

  IF p_channel_listing_id IS NOT NULL THEN
    UPDATE public.channel_listing
    SET
      price_decision_snapshot_id = v_snapshot_id,
      linked_floor_price = COALESCE(NULLIF(v_quote->>''floor_price'', '''')::numeric, linked_floor_price),
      linked_target_price = COALESCE(NULLIF(v_quote->>''target_price'', '''')::numeric, linked_target_price),
      linked_ceiling_price = COALESCE(NULLIF(v_quote->>''ceiling_price'', '''')::numeric, linked_ceiling_price),
      last_price_snapshot_at = now()
