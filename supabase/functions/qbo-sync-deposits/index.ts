 ...
 *
 * Fetches all Deposit records from QBO and lands raw payloads into
 * landing_raw_qbo_deposit. Processing happens in qbo-process-pending.
 */
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const qboDeposits = await queryQboAll(baseUrl, accessToken, "Deposit");
    console.log(`Landing ${qboDeposits.length} QBO deposits (correlation: ${correlationId})`);

    let landed = 0, skipped = 0;

    for (const d of qboDeposits) {
      try {
        const externalId = String(d.Id);
        const { data: existing } = await admin.from("landing_raw_qbo_deposit")
          .select("id, status, raw_payload").eq("external_id", externalId).maybeSingle();

        if (existing) {
          const payloadChanged = JSON.stringify(existing.raw_payload) !== JSON.stringify(d);
          if (payloadChanged) {
            const newStatus = existing.status === "committed" ? "pending" : existing.status;
            await admin.from("landing_raw_qbo_deposit").update({
              raw_payload: d, received_at: new Date().toISOString(), status: newStatus,
            }).eq("id", existing.id);
          ...
          await admin.from("landing_raw_qbo_deposit").insert({
            external_id: externalId, raw_payload: d, status: "pending",
            correlation_id: correlationId, received_at: new Date().toISOString(),
          });
          landed++;
        }
      } catch (err) {
        console.error(`Failed to land deposit ${d.Id}:`, err);
      }
    }

    return new Response(
      JSON.stringify({ success: true, total: qboDeposits.length, landed, skipped }),
      { headers: { ...corsHeaders, "Content-Type": "application/json" } },
    );
  } catch (err) {
    console.error("qbo-sync-deposits error:", err);
    return new Response(
