...
/**
 * qbo-sync-vendors — LAND ONLY
 *
 * Fetches all vendors from QBO and lands raw payloads into
 * landing_raw_qbo_vendor. No canonical table writes.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const qboVendors = await queryQboAll(baseUrl, accessToken, "Vendor");
    console.log(`Landing ${qboVendors.length} QBO vendors (correlation: ${correlationId})`);

    let landed = 0, skipped = 0;

    for (const vendor of qboVendors) {
      try {
        const externalId = String(vendor.Id);
        const { data: existing } = await admin.from("landing_raw_qbo_vendor")
          .select("id, status, raw_payload").eq("external_id", externalId).maybeSingle();

        if (existing) {
          const payloadChanged = JSON.stringify(existing.raw_payload) !== JSON.stringify(vendor);
          if (payloadChanged) {
            const newStatus = existing.status === "committed" ? "pending" : existing.status;
            await admin.from("landing_raw_qbo_vendor").update({
              raw_payload: vendor, received_at: new Date().toISOString(), status: newStatus,
            }).eq("id", existing.id);
          ...
          await admin.from("landing_raw_qbo_vendor").insert({
            external_id: externalId, raw_payload: vendor, status: "pending",
            correlation_id: correlationId, received_at: new Date().toISOString(),
          });
          landed++;
        }
      } catch (err) {
        console.error(`Failed to land vendor ${vendor.Id}:`, err);
      }
    }

    return new Response(
      JSON.stringify({
        success: true, total: qboVendors.length, landed, skipped,
      }),
      { headers: { ...corsHeaders, "Content-Type": "application/json" } },
    );
  } catch (err) {
    console.error("qbo-sync-vendors error:", err);
    return new Response(
